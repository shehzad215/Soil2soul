...
(function() {
	...
	d.AxisBase = d.Class({
		...
		parseDatesDraw: function() {
			...
			var w, x = this.gridAlpha,
				y, u = this.choosePeriod(0),
				A = u.period,
				u = u.count,
			...
			var C = 0,
				J = 0;
			a && 1 < u && (w = this.chooseMinorFrequency(u), y = d.getPeriodDuration(A, w), "DD" == A && (y += d.getPeriodDuration("hh")), "fff" == A && (y = 1));
			if (0 < this.gridCountR)
				for (B - 5 - k > this.autoRotateCount && !isNaN(this.autoRotateAngle) && (this.labelRotationR = this.autoRotateAngle), a = k; a <= B; a++) {
					...
					} else this.rotate ? "date" == this.type && "middle" == this.gridPosition && (J = -z * this.stepWidth / 2) : "date" == this.type && (h = this.axisWidth - h);
					f = !1;
					...
					m = l = p;
					if (!isNaN(w))
						for (h = 1; h < u; h += w) this.gridAlpha = this.minorGridAlpha, f = p + y * h, f = d.resetDateToMin(new Date(f), A, w, t).getTime(), f = new this.axisItemRenderer(this, (f - this.startTime) * this.stepWidth, void 0, void 0, void 0, void 0, void 0, void 0, void 0, !0), this.pushAxisItem(f);
					this.gridAlpha = x
				}
		},
...
(function() {
	...
	d.ValueAxis = d.Class({
		...
		draw: function() {
			...
			if (isNaN(this.min) || isNaN(this.max) || !this.foundGraphs || Infinity == this.min || -Infinity == this.max) e = !0;
				...
				if (this.logarithmic) {
					...
					this.relativeStart =
						(Math.log(this.minReal) / Math.LN10 - Math.log(this.fullMin) * Math.LOG10E) / g;
				...
				var r = Math.round((this.maxCalc - this.minCalc) / this.step) + 1,
					w;
				!0 === h ? (w = Math.log(this.max) * Math.LOG10E - Math.log(this.minReal) * Math.LOG10E, this.stepWidth = this.axisWidth / w, w > this.logGridLimit && (r = Math.ceil(Math.log(this.max) * Math.LOG10E) +
					1, t = Math.round(Math.log(this.minReal) * Math.LOG10E), r > this.gridCountR && (p = Math.ceil(r / this.gridCountR)))) : this.stepWidth = this.axisWidth / (this.max - this.min);
				var x = 0;
				...
				if (this.autoGridCount || 0 !== this.gridCount)
					if ("date" == c) this.generateDFObject(), this.timeDifference = this.max - this.min, this.maxTime = this.lastTime = this.max, this.startTime = this.firstTime = this.min, this.parseDatesDraw();
						...
						for (r >= this.autoRotateCount && !isNaN(this.autoRotateAngle) && (this.labelRotationR = this.autoRotateAngle), c = this.minCalc, h && (r++, c = this.maxCalc - r * x), this.gridCountReal = r, g = this.startCount = t; g < r; g += p)
							if (t = x * g + c, t = d.roundTo(t, this.maxDecCount + 1), !this.integersOnly || Math.round(t) == t)
								if (isNaN(y) || Number(d.toFixed(t, y)) == t) {
									if (!0 === h)
										if (w > this.logGridLimit) t = Math.pow(10, g);
									...
									if (t == this.baseValue && "radar" != a.type) {
										...
										u = this.width;
										var J = this.height;
										"H" == this.orientation ? 0 <= f && f <= u + 1 && (D = [f, f, f + k], C = [J, 0, l]) : 0 <= f && f <= J + 1 && (D = [0, u, u + k], C = [f, f, f + l]);
										D && (u = d.fitToBounds(2 * this.gridAlpha, 0, 1), isNaN(this.zeroGridAlpha) || (u = this.zeroGridAlpha), u = d.line(a.container, D, C, this.gridColor, u, 1, this.dashLength), u.translate(this.x, this.y), this.grid0 = u, a.axesSet.push(u), u.toBack(), d.setCN(a, u, this.bcn + "zero-grid-" + this.id), d.setCN(a, u, this.bcn + "zero-grid"))
									}
									if (!isNaN(z) && 0 < A && g < r - 1) {
										u = x / z;
										h && (z = x * (g + p) + this.minCalc, z = d.roundTo(z, this.maxDecCount + 1), w > this.logGridLimit && (z = Math.pow(10, g + p)), u = 9, z = (z - t) / u);
										f = this.gridAlpha;
										this.gridAlpha = this.minorGridAlpha;
										for (J = 1; J < u; J++) {
											var H = this.getCoordinate(t + z * J),
												H = new this.axisItemRenderer(this, H, "", !1, 0,
													0, !1, !1, 0, !0);
											this.pushAxisItem(H)
										}
										this.gridAlpha = f
		...
		stackGraphs: function(a) {
			...
			for (p = n; p <= l; p++) {
				...
				for (q = 0; q < k.length; q++)
					if (n = k[q], n.hidden) n.newStack && (h[p] = NaN, e[p] = NaN);
					else if (m = n.type, n.chart == this.chart && n.valueAxis == this && a == m && n.stackable)
					if (m = this.data[p].axes[this.id].graphs[n.id], g = m.values.value, isNaN(g)) n.newStack && (h[p] = NaN, e[p] = NaN);
					else {
			...
			for (p = this.start; p <= this.end; p++)
				for (q = 0; q < k.length; q++)(n = k[q], n.hidden) ? n.newStack && (h[p] = NaN, e[p] = NaN) : (m = n.type, n.chart == this.chart && n.valueAxis == this && a == m && n.stackable && (m = this.data[p].axes[this.id].graphs[n.id], g = m.values.value, isNaN(g) || (c = g / f[p] * 100, m.values.percents = c, m.values.total = f[p], n.newStack && (h[p] = NaN, e[p] = NaN), "100%" == b && (isNaN(e[p]) && (e[p] = 0), isNaN(h[p]) && (h[p] = 0), 0 > c ? (m.values.close = d.fitToBounds(c + e[p], -100, 100), m.values.open = e[p], e[p] = m.values.close) : (m.values.close = d.fitToBounds(c +
					h[p], -100, 100), m.values.open = h[p], h[p] = m.values.close)))))
		},
		...
		zoomToValues: function(a, b, c) {
			...
			if (this.minZoom != a || this.maxZoom != b) return this.minZoom =
				a, this.maxZoom = b, e = {
					type: "axisZoomed"
		...
		getCoordinate: function(a, b) {
			...
			var c = this.rotate,
				...
				g = this.min,
				k = this.minReal;
			!0 === this.logarithmic ? (0 === a && (a = this.treatZeroAs), g = Math.log(a) * Math.LOG10E - Math.log(k) * Math.LOG10E, c = c ? !0 === e ? d - f * g : f * g : !0 === e ? f * g : d - f * g) : c = !0 === e ?
				c ? d - f * (a - g) : f * (a - g) : c ? f * (a - g) : d - f * (a - g);
...
(function() {
	...
	d.RecItem = d.Class({
		construct: function(a, b, c, e, h, f, g, k, l, m, n, q) {
			...
			"V" == Oa && (n = 0);
			var ca;
			Q && "" !== c && (ca = a.autoWrap && 0 === n ? d.wrappedText(x, c, w, t, r, ga, k, Math.abs(h), 0) : d.text(x, c, w, t, r, ga, k), ga = ca.getBBox(), ha = ga.width, Pa = ga.height);
			if ("H" == Oa) {
				if (0 <= b && b <= Aa + 1 && (0 < B && 0 < z && b + l <= Aa + 1 && (fa = d.line(x, [b + l, b + l], [0, B], A, z, C), y.push(fa)), 0 < D && (Ba = d.line(x, [b, b + va, b + va], [X, X + ma, ma], J, D, C, H), y.push(Ba))), v = 0, G = b, g && 90 == n && I && (G -= r), !1 === e ? (ga = "start", v = "bottom" == Z ? I ? v + B : v - B : I ? v - B : v + B, G += 3, 0 < h && (G += h / 2 - 3, ga = "middle"), 0 < n && (ga = "middle")) : ga = "middle", 1 == ia && 0 < O && !g && !m && da < Aa && (e = d.fitToBounds(b, 0, Aa), da = d.fitToBounds(da, 0, Aa), Y = e - da, 0 < Y && (Da = d.rect(x, Y, a.height, S, O), Da.translate(e - Y + va, ma), y.push(Da))), "bottom" == Z ? (v += X + r / 2 + ea, I ? (0 < n ? (v = X - ha / 2 * Math.sin(T) - B - 3, G += ha / 2 * Math.cos(T) - 4 + 2) : 0 > n ? (v = X + ha * Math.sin(T) - B - 3 + 2, G += -ha * Math.cos(T) - Pa * Math.sin(T) - 4) : v -= B + r + 3 + 3, v -= aa) : (0 < n ? (v = X + ha / 2 * Math.sin(T) + B + 3, G -= ha / 2 * Math.cos(T)) : 0 > n ? (v = X + B + 3 - ha / 2 * Math.sin(T) + 2, G += ha / 2 * Math.cos(T)) : v += B + u + 3 + 3, v += aa)) : (v += ma + r / 2 - ea, G += va, I ? (0 < n ? (v = ha / 2 * Math.sin(T) + B + 3, G -= ha / 2 * Math.cos(T)) : v += B + 3, v += aa) : (0 < n ? (v = -(ha / 2) * Math.sin(T) - B - 6, G += ha / 2 * Math.cos(T)) : v -= B + r + 3 + u + 3, v -= aa)), "bottom" == Z ? Y = (I ? X - B - 1 : X + u - 1) + ea : (na = va, Y = (I ? ma : ma - B - u + 1) - ea), f && (G += f), r = G, 0 < n && (r += ha / 2 * Math.cos(T)), ca && (f = 0, I && (f = ha / 2 * Math.cos(T)), r + f > Aa + 2 || 0 > r)) ca.remove(), ca = null
			} else {
				0 <= b && b <= X + 1 && (0 < B && 0 < z && b + l <= X + 1 && (fa = d.line(x, [0, B + 1], [b + l, b + l], A, z, C), y.push(fa)), 0 < D && (Ba = d.line(x, [0, va, Aa + va], [b, b + ma, b + ma], J, D, C, H), y.push(Ba)));
				ga = "end";
				if (!0 === I && "left" == Z || !1 === I && "right" == Z) ga = "start";
				v = b - Pa / 2 + 2;
				1 == ia && 0 < O && !g && !m && (e = d.fitToBounds(b, 0, X), da = d.fitToBounds(da, 0, X), T = e - da, Da = d.polygon(x, [0, a.width, a.width, 0], [0, 0, T, T], S, O), Da.translate(va, e - T + ma), y.push(Da));
				v += r / 2;
				"right" == Z ? (G += va + Aa + ea, v += ma, I ? (f || (v -= r / 2 + 3), G = G - (B + 4) - aa) : (G += B + 4 + u, v -= 2, G += aa)) : I ? (G += B + 4 - ea, f || (v -= r / 2 + 3), g && (G += va, v += ma), G += aa) : (G += -B - u - 4 - 2 - ea, v -= 2, G -= aa);
				fa && ("right" == Z ? (na += va + ea + Aa - 1, Y += ma, na = I ? na - u : na + u) : (na -= ea, I || (na -= B + u)));
				f && (v += f);
				I = -3;
				"right" == Z && (I += ma);
			...
			ca && (ca.attr({
				"text-anchor": ga
			}), ca.translate(G, v, NaN, !0), 0 !== n && ca.rotate(-n, a.chart.backgroundColor), a.allLabels.push(ca), this.label = ca, d.setCN(p, ca, a.bcn + "label"), d.setCN(p, ca, q, !0), g && d.setCN(p, ca, "guide"));
			Ba && (d.setCN(p, Ba, a.bcn + "grid"), d.setCN(p, Ba, q, !0), g && d.setCN(p, Ba, "guide"));
		...
		updateAnimations: function() {
			...
			if (this.animations)
				for (a = this.animations.length - 1; 0 <= a; a--) {
					...
					} else h.node.style[f] = Number(b.to) +
...
(function() {
	...
	d.AmGraph = d.Class({
		...
		createRadarGraph: function() {
			...
			for (m = this.start; m <= this.end; m++) {
				...
				if (isNaN(q)) this.connect || (this.drawLineGraph(b, c, e, d), b = [], c = [], e = [], d = []);
					...
					n.x = q;
					n.y = t;
					if (x = this.labelText) x = this.createLabel(n, x), w = 0, r && (w = r.size), this.positionLabel(n, q, t, x, w);
					isNaN(f) && (f = q);
					isNaN(g) && (g = t)
				}
			}
			...
			if (e) {
				...
				switch (m) {
					...
					case "left":
						...
						break;
					case "inside":
						"column" == this.type && (l = !0, q ? p ? (k = "end", b = r - 3 - x) : (k = "start", b = r + 3 + x) : c = p ? w + 7 + x : w - 10 - x);
						break;
					case "middle":
						"column" == this.type && (l = !0, q ? b -= (b - r) / 2 + x - 3 : c -= (c - w) / 2 + x - 3)
				}
				...
				if (e && "radar" != f.type)
					if (q) {
						...
						!this.showAllValueLabels && (0 > b || b > this.width) && (e.remove(), e = null)
					} else {
						if (0 > b || b > this.width) e.remove(),
							e = null;
						!this.showAllValueLabels && (0 > c || c > this.height) && (e.remove(), e = null)
					}
		...
		createSerialGraph: function() {
			...
			if (!(I || "line" != g && "smoothedLine" != g && "step" != g || (1 == e.length && "step" != g && "none" == this.bullet && (this.bullet = "round", this.resetBullet = !0), !Qa && void 0 == Ja || m))) {
				var Ua = Oa;
				Ua > f.max && (Ua = f.max);
				Ua < f.min && (Ua = f.min);
				...
				t ? (ha = q, Pa = Math.abs(Lb - Ka), ub = q, vb = Math.abs(ib - Ka), hb = tb = 0, f.reversed ? (ca = 0, gb = Ka) : (ca = Ka, gb = 0)) : (Pa = n, ha = Math.abs(Lb - Ka), vb = n, ub = Math.abs(ib - Ka), gb = ca = 0, f.reversed ? (hb = p, tb = Ka) : hb = Ka)
			}
			var La = Math.round;
			this.pmx = La(ca);
			this.pmy = La(tb);
			...
			if ("line" == g || "step" == g || "smoothedLine" == g) {
				if (0 < Q) {
					for (L = Q - 1; - 1 < L; L--)
						if (G = e[L], v = G.axes[f.id].graphs[b], na = v.values.value, !isNaN(na)) {
							Q = L;
							break
						}
			...
			var U = [],
				...
				Ra = !1;
			if ("line" == g || "step" == g || "smoothedLine" == g)
				if (this.stackable && "regular" == O || "100%" == O || this.fillToGraph) Ra = !0;
			...
			for (L = Q; L <= ia; L++) {
				...
				"none" == O && (Z = isNaN(v.columnIndex) ? this.columnIndex : v.columnIndex);
				if (Ia) {
					...
					var ya = Math.floor(Y / 2) + Math.floor(Wa * Y / 2),
						Ha = ya,
						nb = 0;
					"left" == this.stepDirection && (nb = (2 * Y + Wa * Y) / 2, ja -= nb);
					"center" == this.stepDirection && (nb = Y / 2, ja -= nb);
					"start" == this.pointPosition && (ja -= Y / 2 + Math.floor(Wa * Y / 2), ya = 0, Ha = Math.floor(Y) + Math.floor(Wa * Y));
					"end" == this.pointPosition && (ja += Y / 2 + Math.floor(Wa * Y / 2), ya = Math.floor(Y) + Math.floor(Wa * Y), Ha = 0);
					if (Nb) {
						var Cb = this.columnWidth;
						isNaN(Cb) || (ya *= Cb, Ha *= Cb)
					}
					I || (v.x = ja); - 1E5 > ja && (ja = -1E5);
					ja > n + 1E5 && (ja = n + 1E5);
					t ? (E = F, N = M, M = F = ja, isNaN(ta) && !this.fillToGraph &&
						(N = ma), pa = qa, ra = sa) : (N = E = ja, isNaN(ta) && !this.fillToGraph && (M = ma));
					if (!Bb && W < ta || Bb && W < Ba) v.isNegative = !0, Qa && (Ma = Qa), Za && (Va = Za), void 0 != Ja && (Ga = Ja);
					$a = !1;
					isNaN(W) || (m ? W > Ta ? (Sa && ($a = !0), Sa = !1) : (Sa || ($a = !0), Sa = !0) : v.isNegative = W < Oa ? !0 : !1, Ba = W);
					var Ob = !1;
					I && a.chartScrollbar.ignoreCustomColors && (Ob = !0);
					Ob || (void 0 != v.color && (Ma = v.color), v.fillColors && (Ma = v.fillColors));
					switch (g) {
						case "line":
							if (isNaN(W)) x || (this.drawLineGraph(y, u, U, V), y = [], u = [], U = [], V = []);
							else {
								if (Math.abs(E - jb) >= lb || Math.abs(F - kb) >= lb) y.push(E), u.push(F), jb = E, kb = F;
								wa = E;
								Ea = F;
								ka = E;
								la = F;
								...
								v.gap && (this.drawLineGraph(y, u, U, V), y = [], u = [], U = [], V = [])
							}
							break;
						case "smoothedLine":
							if (isNaN(W)) x || (this.drawSmoothedGraph(y, u, U, V), y = [], u = [], U = [], V = []);
							else {
								if (Math.abs(E - jb) >= lb || Math.abs(F - kb) >= lb) y.push(E), u.push(F), jb = E, kb = F;
								wa = E;
								Ea = F;
								...
								la = F;
								!Ra || isNaN(M) || isNaN(N) || (U.push(N), V.push(M));
								void 0 == v.lineColor && void 0 == v.fillColors && isNaN(v.dashLength) || (this.drawSmoothedGraph(y, u, U, V), y = [E], u = [F], U = [], V = [], !Ra || isNaN(M) || isNaN(N) || (U.push(N), V.push(M)), this.lineColorSwitched = v.lineColor, this.fillColorsSwitched = v.fillColors, this.dashLengthSwitched = v.dashLength);
								v.gap && (this.drawSmoothedGraph(y, u, U, V), y = [], u = [], U = [], V = [])
							}
							break;
						case "step":
							if (!isNaN(W)) {
								t ? (isNaN(A) || (y.push(A), u.push(F - ya)), u.push(F - ya), y.push(E), u.push(F + Ha), y.push(E), !Ra || isNaN(M) || isNaN(N) || (isNaN(B) || (U.push(B), V.push(M - ya)), U.push(N), V.push(M - ya), U.push(N), V.push(M + Ha))) : (isNaN(z) || (u.push(z), y.push(E - ya)), y.push(E - ya), u.push(F), y.push(E + Ha), u.push(F), !Ra || isNaN(M) || isNaN(N) || (isNaN(D) || (U.push(N - ya), V.push(D)), U.push(N - ya), V.push(M), U.push(N + Ha), V.push(M)));
								A = E;
								z = F;
								B = N;
								D = M;
								...
								if ($a || void 0 != v.lineColor || void 0 != v.fillColors || !isNaN(v.dashLength)) {
									...
									y.pop();
									u.pop();
									this.drawLineGraph(y, u, U, V);
									y = [cc];
									u = [dc];
									t ? (u.push(F + Ha), y.push(E)) : (y.push(E + Ha), u.push(F));
									U = [];
									V = [];
									this.lineColorSwitched = v.lineColor;
									this.fillColorsSwitched = v.fillColors;
									this.dashLengthSwitched = v.dashLength;
									m && (Sa ? (this.lineColorSwitched = Aa, this.fillColorsSwitched = wb) : (this.lineColorSwitched = Ja, this.fillColorsSwitched = Qa))
								}
								if (Nb || v.gap) A = z = NaN, this.drawLineGraph(y, u, U, V), y = [], u = [], U = [], V = []
							} else if (!x) {
								if (1 >= this.periodSpan || 1 < this.periodSpan && E - A > ya + Ha) A = z = NaN;
								this.drawLineGraph(y, u, U, V);
								y = [];
								u = [];
								U = [];
						...
						case "column":
							...
							if (!isNaN(W)) {
								...
								v.isNegative && (Qa && (Ma = Qa), void 0 != Ja && (Ca = Ja));
								var Pb = f.min,
									Qb = f.max,
									ob = ta;
								isNaN(ob) && (ob = Oa);
								if (!(W < Pb && ob < Pb || W > Qb && ob > Qb)) {
									var za;
									if (t) {
										"3d" == O ? (P = F - (r / 2 - this.depthCount + 1) * (k + T) + T / 2 + S * Z, R = N + H * Z, za = Z) : (P = Math.floor(F - (r / 2 - Z) * (k + T) + T / 2), R = N, za = 0);
										K = k;
										wa = E;
										Ea = P + k / 2;
										ka = E;
										la = P + k / 2;
										P + K > q + za * S && (K = q - P + za * S);
										P < za * S && (K += P, P = za * S);
										ba = E - N;
										var ec = R;
										R = d.fitToBounds(R, 0, n);
										ba += ec - R;
										ba = d.fitToBounds(ba, -R, n - R + H * Z);
										v.labelIsNegative = 0 > ba ? !0 : !1;
										0 === ba && 1 / W === 1 / -0 && (v.labelIsNegative = !0);
										isNaN(G.percentWidthValue) || (K = this.height * G.percentWidthValue / 100, P = ja - K / 2, Ea = P + K / 2);
										K = d.roundTo(K, 2);
										ba = d.roundTo(ba, 2);
										P < q && 0 < K && (ua = new d.Cuboid(h, ba, K, H - a.d3x, S - a.d3y, Ma, Va, da, Ca, X, ga, w, t, ea, bb, mb, aa), v.columnWidth = Math.abs(ba), v.columnHeight = Math.abs(K))
									} else {
										"3d" == O ? (R = E - (r / 2 - this.depthCount + 1) * (k + T) + T / 2 + H * Z, P = M + S * Z, za = Z) : (R = E - (r / 2 - Z) * (k + T) + T / 2, P = M, za = 0);
										K = k;
										wa = R + k / 2;
										Ea = F;
										ka = R + k / 2;
										la = F;
										R + K > n + za * H && (K = n - R + za * H);
										R < za * H && (K += R - za * H, R = za * H);
										ba = F - M;
										v.labelIsNegative = 0 < ba ? !0 : !1;
										0 === ba && -0 === W && (v.labelIsNegative = !0);
										var fc = P;
										P = d.fitToBounds(P, this.dy, q);
										ba += fc - P;
										ba = d.fitToBounds(ba, -P + S * za, q - P);
										isNaN(G.percentWidthValue) || (K = this.width * G.percentWidthValue /
											100, R = ja - K / 2, wa = R + K / 2);
										K = d.roundTo(K, 2);
										ba = d.roundTo(ba, 2);
										R < n + Z * H && 0 < K && (this.showOnAxis && (P -= S / 2), ua = new d.Cuboid(h, K, ba, H - a.d3x, S - a.d3y, Ma, Va, da, Ca, this.lineAlpha, ga, w, t, ea, bb, mb, aa), v.columnHeight = Math.abs(ba), v.columnWidth = Math.abs(K))
									}
								}
								if (ua) {
									xa = ua.set;
									...
									P = d.roundTo(P, 2);
									xa.translate(R, P);
									(v.url || this.showHandOnHover) && xa.setAttr("cursor", "pointer");
									if (!I) {
										"none" == O && (J = t ? (this.end + 1 - L) * C - c : C * L + c);
										"3d" == O && (t ? (J = (this.end + 1 - L) * C - c - 1E3 * this.depthCount, wa += H * Z, ka += H * Z, v.y += H * Z) : (J = (C - c) * (L + 1) + 1E3 * this.depthCount, Ea += S * Z, la += S * Z, v.y += S * Z));
										if ("regular" == O || "100%" == O) J = t ? 0 < Ia.value ? (this.end + 1 - L) * C + c : (this.end + 1 - L) * C - c : 0 < Ia.value ? C * L + c : C * L - c;
										...
										this.animateColumns(ua, L, E, N, F, M);
										this.addListeners(xa, v);
										void 0 !== this.tabIndex && xa.setAttr("tabindex", this.tabIndex)
									}
									this.columnsSet.push(xa)
								}
							}
							break;
						case "candlestick":
							if (!isNaN(ta) && !isNaN(W)) {
								...
								if (t) {
									...
									if (P < q && 0 < K) {
										...
										!isNaN(ra) && !isNaN(pa) && F < q && 0 < F && (Xa = d.line(h, Db, [F, F], Ca, X, da), cb = d.line(h, Eb, [F, F], Ca, X, da));
										ba = E - N;
										ua = new d.Cuboid(h, ba, K, H, S, Ma, Ya, da, Ca, X, ga, w, t, ea, bb, mb, aa)
									}
								} else {
									"open" == l && (la = M);
									"high" == l && (la = sa);
									...
									if (R < n && 0 < K) {
										Bb && W >= ta && (Va = 0);
										var ua = new d.Cuboid(h, K, ba, H, S, Ma, Va, da, Ca, X, ga, w, t, ea, bb, mb, aa),
											Fb, Gb;
										W > ta ? (Fb = [F, sa], Gb = [M, qa]) : (Fb = [M, sa], Gb = [F, qa]);
										!isNaN(sa) && !isNaN(qa) && E < n && 0 < E && (Xa = d.line(h, [E, E], Fb, Ca, X, da), cb = d.line(h, [E, E], Gb, Ca, X, da), d.setCN(a, Xa, this.bcn + "line-high"), v.className && d.setCN(a, Xa, v.className, !0), d.setCN(a, cb, this.bcn + "line-low"), v.className && d.setCN(a, cb, v.className, !0))
									}
								}
								ua && (xa = ua.set, v.columnGraphics = xa, fa.push(xa), xa.translate(R, P - da / 2), (v.url || this.showHandOnHover) && xa.setAttr("cursor", "pointer"), Xa && (fa.push(Xa), fa.push(cb)), I || (v.x = t ? P + K / 2 : R + K / 2, this.animateColumns(ua, L, E, N, F, M), this.addListeners(xa, v), void 0 !== this.tabIndex && xa.setAttr("tabindex", this.tabIndex)))
							}
							break;
						case "ohlc":
							if (!(isNaN(ta) || isNaN(zb) || isNaN(Ab) || isNaN(W))) {
								var Rb = h.set();
								fa.push(Rb);
								W < ta && (v.isNegative = !0, void 0 != Ja && (Ga = Ja));
								void 0 != v.lineColor && (Ga = v.lineColor);
								var pb, qb, rb;
								if (t) {
									la = F;
									ka = E;
									...
									0 <= N && N <= oa && (qb = d.line(h, [N, N], [Hb, Sb], Ga, X, da, ea));
									0 < F && F < q && (pb = d.line(h, [pa, ra], [F, F], Ga, X, da, ea));
									0 <= E && E <= oa && (rb = d.line(h, [E, E], [Sb, Ib], Ga, X, da, ea))
								} else {
									la = F;
									"open" == l && (la = M);
									"high" == l && (la = sa);
									...
									0 <= M && M <= Fa && (qb = d.line(h, [Jb, Tb], [M, M], Ga, X, da, ea));
									0 < E && E < n && (pb = d.line(h, [E, E], [qa, sa], Ga, X, da, ea));
									0 <= F && F <= Fa && (rb = d.line(h, [Tb, Kb], [F, F], Ga, X, da, ea))
								}
								fa.push(qb);
								fa.push(pb);
								fa.push(rb);
					...
					if (!I && !isNaN(W)) {
						...
						if (this.end - this.start <= Ub || 0 === Ub) {
							var Vb = this.createBullet(v, ka, la, L),
								Wb = this.labelText;
							if (Wb && !isNaN(wa) && !isNaN(wa)) {
								var gc = this.createLabel(v, Wb),
									Xb = 0;
								Vb && (Xb = Vb.size);
								this.positionLabel(v, wa, Ea, gc, Xb)
							}
							if ("regular" == O || "100%" == O) {
								var Yb = f.totalText;
								if (Yb) {
									...
									if (Na) {
										...
										t ? (eb = Ea, db = 0 > W ? E - $b / 2 - 2 - fb - sb : E + $b / 2 + 3 + fb + sb) : (db = wa, eb = 0 > W ? F + ac / 2 + fb + sb : F - ac / 2 - 3 - fb - sb);
										Na.translate(db, eb);
										f.totals[L] = Na;
			...
			this.lastDataItem = v;
			if ("line" == g || "step" == g || "smoothedLine" == g) "smoothedLine" == g ? this.drawSmoothedGraph(y, u, U, V) : this.drawLineGraph(y, u, U, V), I || this.launchAnimation();
		...
		drawLineGraph: function(a, b, c, e) {
			...
			if (1 < a.length) {
				...
				void 0 === x || h.useNegativeColorIfDown || (p = d.line(l, a, b, x, r, p, y, !1, !0, f), p.node.setAttribute("stroke-linejoin", "round"), d.setCN(k, p, h.bcn + "stroke"), d.setCN(k, p, h.bcn + "stroke-negative"), n.push(p));
				if (0 < g || 0 < A)
					if (p = a.join(";").split(";"), r = b.join(";").split(";"), q = k.type, "serial" == q || "radar" == q ? 0 < c.length ? (c.reverse(), e.reverse(), p = a.concat(c), r = b.concat(e)) : "radar" == q ? (r.push(0), p.push(0)) : h.rotate ? (r.push(r[r.length - 1]), p.push(z), r.push(r[0]), p.push(z), r.push(r[0]), p.push(p[0])) : (p.push(p[p.length -
							1]), r.push(z), p.push(p[0]), r.push(z), p.push(a[0]), r.push(r[0])) : "xy" == q && (b = h.fillToAxis) && (d.isString(b) && (b = k.getValueAxisById(b)), "H" == b.orientation ? (z = "top" == b.position ? 0 : b.height, p.push(p[p.length - 1]), r.push(z), p.push(p[0]), r.push(z), p.push(a[0]), r.push(r[0])) : (z = "left" == b.position ? 0 : b.width, r.push(r[r.length - 1]), p.push(z), r.push(r[0]), p.push(z), r.push(r[0]), p.push(p[0]))), a = h.gradientRotation, 0 < g && (b = d.polygon(l, p, r, w, g, 1, "#000", 0, a, f), b.pattern(h.pattern, NaN, k.path), d.setCN(k, b, h.bcn + "fill"), m.push(b)), u || void 0 !== x) isNaN(A) && (A = g), u || (u = x), f = d.polygon(l, p, r, u, A, 1, "#000", 0, a, f), d.setCN(k, f, h.bcn + "fill"), d.setCN(k, f, h.bcn + "fill-negative"), f.pattern(h.pattern, NaN, k.path), n.push(f), n.click(function(a) {
						h.handleGraphEvent(a, "clickGraph")
					}).mouseover(function(a) {
						h.handleGraphEvent(a, "rollOverGraph")
					}).mouseout(function(a) {
		...
		drawSmoothedGraph: function(a, b, c, e) {
			if (1 < a.length) {
				...
				m = new d.Bezier(g, a, b, p, m, n, t, 0, h, void 0, u);
				d.setCN(f, m, this.bcn + "stroke");
				k.push(m.path);
				void 0 !== r && (n = new d.Bezier(g, a, b, r, A, n, t, 0, h, void 0, u), d.setCN(f, n, this.bcn + "stroke"), d.setCN(f, n, this.bcn + "stroke-negative"), l.push(n.path));
				0 < q && (m = a.join(";").split(";"), p = b.join(";").split(";"), n = "", 0 < c.length ? (c.push("M"), e.push("M"), c.reverse(), e.reverse(), m = a.concat(c), p = b.concat(e)) : (this.rotate ? (n += " L" + y + "," + b[b.length - 1], n += " L" + y + "," + b[0]) : (n += " L" + a[a.length - 1] + "," + y, n += " L" + a[0] + "," + y), n += " L" + a[0] + "," + b[0]), c = new d.Bezier(g, m, p, NaN, 0, 0, t, q, h, n, u), d.setCN(f, c, this.bcn + "fill"), c.path.pattern(this.pattern, NaN, f.path), k.push(c.path), w || void 0 !== r) && (x || (x = q), w || (w = r), a = new d.Bezier(g, a, b, NaN, 0, 0, w, x, h, n, u), a.path.pattern(this.pattern, NaN, f.path), d.setCN(f, a, this.bcn + "fill"), d.setCN(f, a, this.bcn + "fill-negative"), l.push(a.path));
				this.applyMask(l, k)
			}
		},
			...
			if (!isNaN(b) && !isNaN(c) && ("none" != this.bullet || this.customBullet || a.bullet || a.customBullet)) {
				...
				if (p) {
					...
					if ("serial" == e.type || "gantt" == e.type)
						if (-.5 > b || b > this.width || c < -g / 2 || c > this.height) p.remove(), p = null;
		...
		dispatchPanEvent: function() {
			if (this.mouseIsDown) {
				var a = d.roundTo((this.mouseX - this.mouseX0) / this.width, 3),
					...
					c =
					d.roundTo((this.mouse2X - this.mouse2X0) / this.width, 3),
				...
				h && (this.hideLines(), this.fire({
						...
						delta2Y: e,
						index: this.index
					}), this.prevDeltaX = a, this.prevDeltaY = b, this.prevDelta2X = c,
					this.prevDelta2Y = e)
			}
		},
...
(function() {
	...
	d.SimpleChartScrollbar = d.Class({
		...
		draw: function() {
			...
			if (a.enabled) {
				...
				if ((a.height = g) && f) {
					...
					p.translate(-(m - c) / 2, -(n - g) / 2);
					q.translate(-(m - c) / 2, -(n - g) / 2);
					c = b.set([k, p]);
					b = b.set([l, q]);
					a.iconLeft = c;
					h.push(a.iconLeft);
			...
			if (k) {
				...
				if (n.mouseIsOver) {
					...
					if (this.resizingRight) {
						if (m)
							if (a = r - e, !isNaN(this.maxHeight) && a > this.maxHeight && (a = this.maxHeight), a + e > p + f && (a = p - e + f), 0 > a) this.resizingRight = !1, b = this.resizingLeft = !0;
							else {
								if (0 === a || isNaN(a)) a = .1;
								g.setAttr("height", a)
							}
					...
					if (this.resizingLeft) {
						if (m)
							if (c = e, e = r, e < f && (e = f), isNaN(e) && (e = f), e > p + f && (e = p + f), a = !0 === b ? c - e : k + c - e, !isNaN(this.maxHeight) && a > this.maxHeight && (a = this.maxHeight, e = c), 0 > a) this.resizingRight = !0, this.resizingLeft = !1, g.setAttr("y", c + k - f);
							else {
								if (0 === a || isNaN(a)) a = .1;
								g.setAttr("y", e - f);
								g.setAttr("height", a)
		...
		updateDragIconPositions: function() {
			...
			this.rotate ? (f = this.dragIconWidth, g = this.dragIconHeight, e.translate((k - g) / 2, c - f / 2), d.translate((k - g) / 2, c + a.height - f / 2)) : (f = this.dragIconHeight, g = this.dragIconWidth, e.translate(b - g / 2, (k - f) / 2), d.translate(b - g / 2 + a.width,
				(k - f) / 2))
		},
		...
		dispatchScrollbarEvent: function() {
			if (this.categoryAxis && "ValueAxis" == this.categoryAxis.cname) d.ChartScrollbar.base.dispatchScrollbarEvent.call(this);
				...
				var a = this.chart.chartData,
					b, c;
				b = this.dragger.getBBox();
				var e = b.x,
					h = b.y,
					f = b.width,
					g = b.height,
					k = this.chart;
				this.rotate ? (b = h, c = g) : (b = e, c = f);
				f = {
					type: "zoomed",
					target: this
				};
				...
				var l = this.categoryAxis,
					...
					h = k.maxSelectedTime,
					g = !1;
				if (l.parseDates && !l.equalSpacing) {
					if (a = k.lastTime, k = k.firstTime, l = Math.round(b / m) + k, b = this.dragging ? l + this.timeDifference : Math.round((b + c) / m) + k, l > b && (l = b), 0 < e && b - l < e && (b = Math.round(l + (b - l) / 2), g = Math.round(e / 2), l = b - g, b += g, g = !0), 0 < h && b - l > h && (b = Math.round(l + (b - l) / 2), g = Math.round(h / 2), l = b - g, b += g, g = !0), b > a && (b = a), b - e < l && (l = b - e), l < k && (l = k), l + e > b && (b = l + e), l != this.startTime || b != this.endTime) this.startTime =
						l, this.endTime = b, f.start = l, f.end = b, f.startDate = new Date(l), f.endDate = new Date(b), this.fire(f)
				} else {
					l.startOnAxis || (b += m / 2);
					c -= this.stepWidth / 2;
					var n = l.xToIndex(b),
						q = l.xToIndex(b + c);
					if (n != this.start || this.end != q) l.startOnAxis && (this.resizingRight && n == q && q++, this.resizingLeft && n == q && (0 < n ? n-- : q = 1)), this.start = n, this.end = this.dragging ? this.start + this.difference : q, f.start = this.start, f.end = this.end, l.parseDates && (a[this.start] && (f.startDate = new Date(a[this.start].time)), a[this.end] && (f.endDate = new Date(a[this.end].time))), this.fire(f)
				}
				this.percentStart = n;
				this.percentEnd = q;
		...
		draw: function() {
			...
			if (!isNaN(a)) {
				...
				if (this.show) {
					...
					var J = n.offsetWidth,
						H = n.offsetHeight;
					n.clientHeight && (J = n.clientWidth, H = n.clientHeight);
					A = H + 2 * w;
					C = J + 2 * r;
					!isNaN(D) && C < D && (C = D);
					window.opera && (A += 2);
					...
					"H" != y ? (D = a - C / 2, b < k + A + 10 && "down" != y ? (S = !0, e && (b += z), z = b + x, this.deltaSignY = -1) : (e && (b -= z), z = b - A - x, this.deltaSignY = 1)) : (2 * x > A && (x = A / 2), z = b - A / 2, a < g + (l - g) / 2 ? (D = a + x, this.deltaSignX = -1) : (D = a - C - x, this.deltaSignX = 1));
					z + A >= m && (z = m - A);
					z < k && (z = k);
					D < g && (D = g);
					D + C > l && (D = l - C);
					var k = z + w,
						...
						I, w = this.fillAlpha,
						aa = this.borderAlpha;
					this.showBullet && (I = d.circle(h, ia, t, w), f.push(I));
					this.drop ? (g = C / 1.6, l = 0, "V" == y && (y = "down"), "H" == y && (y = "left"), "down" == y && (D = a + 1, z = b - g - g / 3), "up" == y && (l = 180, D = a + 1, z = b + g + g / 3), "left" == y && (l = 270, D = a + g + g / 3 + 2, z = b), "right" == y && (l = 90, D = a - g - g / 3 + 2, z = b), k = z - H / 2 + 1, m = D - J / 2 - 1, q = d.drop(h, g, l, q, w, r, p, aa)) : 0 < u || 0 === x ? (0 < O && (a = d.rect(h, C, A, q, 0, r +
						1, Q, O, u), d.isModern ? a.translate(1, 1) : a.translate(4, 4), f.push(a)), q = d.rect(h, C, A, q, w, r, p, aa, u)) : (t = [], u = [], "H" != y ? (g = a - D, g > C - x && (g = C - x), g < x && (g = x), t = [0, g - x, a - D, g + x, C, C, 0, 0], u = S ? [0, 0, b - z, 0, 0, A, A, 0] : [A, A, b - z, A, A, 0, 0, A]) : (y = b - z, y > A - x && (y = A - x), y < x && (y = x), u = [0, y - x, b - z, y + x, A, A, 0, 0], t = a < g + (l - g) / 2 ? [0, 0, D < a ? 0 : a - D, 0, 0, C, C, 0] : [C, C, D + C > a ? C : a - D, C, C, 0, 0, C]), 0 < O && (a = d.polygon(h, t, u, q, 0, r, Q, O), a.translate(1, 1), f.push(a)), q = d.polygon(h, t, u, q, w, r, p, aa));
					this.bg = q;
					f.push(q);
					q.toFront();
	...
	d.AmCoordinateChart = d.Class({
		...
		delayParseSerialData: function(a, b) {
			var c = this,
				e = c.processCount;
			setTimeout(function() {
				c.parsePartSerialData.call(c, a, b * e, (b + 1) * e, b)
			}, c.processTimeout)
		},
				...
				for (J = 0; J < C.length; J++) {
					...
					for (S = 0; S < h.length; S++) {
						...
						if (b.valueAxis.id == H) {
							...
							!b.connect && z && z[O] && 0 < Q && B.time - w[O] >= x * Q && (z[O].gap = !0);
...
(function() {
	...
	d.TrendLine = d.Class({
		...
		draw: function() {
			...
			p && !u && (isNaN(x) || (f = p.getCoordinate(x)), isNaN(y) || (g = p.getCoordinate(y)));
...
(function() {
	...
	d.bullet = function(a, b, c, e, h, f, g, k, l, m, n, q, p) {
		...
		switch (b) {
			...
			case "square":
				t = d.polygon(a, [-c /
					2, c / 2, c / 2, -c / 2
				], [c / 2, c / 2, -c / 2, -c / 2], e, h, f, g, k, m - 180, void 0, p);
				break;
			case "rectangle":
	...
	d.drop = function(a, b, c, d, h, f, g, k) {
		...
		b = "M" + n + "," + q + " A" + b + "," + b + ",0,1,1," + p + "," + t + (" A" + r + "," + r + ",0,0,0," + (Math.sin((m + 20) * l) * b + x) + "," + (Math.cos((m + 20) * l) * b + w));
		b += " A" + r + "," + r + ",0,0,0," + n + "," + q;
	...
	d.wedge = function(a, b, c, e, h, f, g, k, l, m, n, q, p, t) {
		...
		e = a.set();
		var O;
		x && (y = r(10 * y), z = r(10 * z), D = r(10 * D), J = r(10 * J), A = r(10 * A), B = r(10 * B), C = r(10 * C), u = r(10 * u), b = r(10 * b), l = r(10 * l), c = r(10 * c), f *= 10, g *= 10, k *= 10, w *= 10, 1 > Math.abs(h) && 1 >= Math.abs(D - z) && 1 >= Math.abs(C - B) && (O = !0));
		h = "";
		var Q;
		q && (H["fill-opacity"] = 0, H["stroke-opacity"] = m["stroke-opacity"] / 2, H.stroke = m.stroke);
		if (0 < l) {
			Q = " M" + y + "," + (A + l) + " L" + z + "," + (B + l);
			x ? (O || (Q += " A" + (b - f) + "," + (l + c - g) + "," + (b + f) + "," + (l + c + g) + "," + z + "," + (B + l) + "," + D + "," + (C + l)), Q += " L" + J + "," + (u + l), 0 < k && (O || (Q += " B" + (b - k) + "," + (l + c - w) + "," + (b + k) + "," +
				(l + c + w) + "," + J + "," + (l + u) + "," + y + "," + (l + A)))) : (Q += " A" + f + "," + g + ",0," + S + ",1," + D + "," + (C + l) + " L" + J + "," + (u + l), 0 < k && (Q += " A" + k + "," + w + ",0," + S + ",0," + y + "," + (A + l)));
			Q += " Z";
			var ia = l;
			x && (ia /= 10);
			for (var I = 0; I < ia; I += 10) {
				var aa = a.path(Q, void 0, void 0, "1000,1000").attr(H);
				e.push(aa);
				aa.translate(0, -I)
			}
			Q = a.path(" M" + y + "," + A + " L" + y + "," + (A + l) + " L" + z + "," + (B + l) + " L" + z + "," + B + " L" + y + "," + A + " Z", void 0, void 0, "1000,1000").attr(H);
			l = a.path(" M" + D + "," + C + " L" + D + "," + (C + l) + " L" + J + "," + (u + l) + " L" + J + "," + u + " L" + D + "," + C + " Z", void 0, void 0, "1000,1000").attr(H);
			e.push(Q);
			e.push(l)
		}
		x ? (O || (h = " A" + r(b - f) + "," + r(c - g) + "," + r(b + f) + "," + r(c + g) + "," + r(z) + "," + r(B) + "," + r(D) + "," + r(C)), g = " M" + r(y) + "," + r(A) + " L" + r(z) + "," + r(B) + h + " L" + r(J) + "," + r(u)) : g = " M" + y + "," + A + " L" + z + "," + B + (" A" + f + "," + g + ",0," + S + ",1," + D + "," + C) + " L" + J + "," + u;
		0 < k && (x ? O || (g += " B" + (b - k) + "," + (c - w) + "," + (b + k) + "," + (c + w) + "," + J + "," + u + "," + y + "," + A) : g += " A" + k + "," + w + ",0," + S + ",0," + y + "," + A);
...
(function() {
	var d = window.AmCharts;
	d.Bezier = d.Class({
		construct: function(a, b, c, e, h, f, g, k, l, m, n) {
			var q, p;
			"object" == typeof g && 1 < g.length && (p = !0, q = g, g = g[0]);
...
(function() {
	...
	d.AmLegend = d.Class({
		...
		arrangeEntries: function() {
			...
			for (A = 0; A < y.length; A++) {
				var J = y[A];
				this.reversedOrder && (J = y[y.length - A - 1]);
				u = J.getBBox();
				var H;
			...
			u = x.getBBox();
			l = u.height + 2 * m - 1;
			"left" == a || "right" == a ? (n = u.width + 2 * f, k = n + b + c, g.style.width = k + "px", this.ieW = k) : n = k - b - c - 1;
		...
		createEntry: function(a) {
			if (!1 !== a.visibleInLegend && !a.hideFromLegend) {
				...
				var g;
				!0 === a.hidden && (g = h = b.markerDisabledColor);
				var k = a.pattern,
					l = a.customMarker;
				l || (l = b.customMarker);
				var m = b.container,
					...
					p = 0,
					t = n / 2;
				if (b.useGraphSettings) {
					e = a.type;
					b.switchType = void 0;
					if ("line" == e || "step" == e || "smoothedLine" == e || "ohlc" == e) k = m.set(), a.hidden || (h = a.lineColorR, g = a.bulletBorderColorR), q = d.line(m, [0, 2 * n], [n / 2, n /
						2
					], h, a.lineAlpha, a.lineThickness, a.dashLength), d.setCN(c, q, "graph-stroke"), k.push(q), a.bullet && (a.hidden || (h = a.bulletColorR), q = d.bullet(m, a.bullet, a.bulletSize, h, a.bulletAlpha, a.bulletBorderThickness, g, a.bulletBorderAlpha)) && (d.setCN(c, q, "graph-bullet"), q.translate(n + 1, n / 2), k.push(q)), t = 0, q = n, p = n / 3;
					else {
						var r;
						a.getGradRotation && (r = a.getGradRotation(), 0 === r && (r = 180));
						q = a.fillColorsR;
						!0 === a.hidden && (q = h);
						if (k = b.createMarker("rectangle", q, a.fillAlphas, a.lineThickness, h, a.lineAlpha, r, k, a.dashLength)) t = n, k.translate(t, n / 2);
						q = n
					}
					d.setCN(c, k, "graph-" + e);
					d.setCN(c, k, "graph-" + a.id)
				} else if (l) k = m.image(l, 0, 0, n, n);
				else {
					var w;
					isNaN(b.gradientRotation) || (w = 180 + b.gradientRotation);
					(k = b.createMarker(e, h, f, void 0, void 0, void 0, w, k)) && k.translate(n / 2, n / 2)
				}
				d.setCN(c, k, "legend-marker");
				b.addListeners(k, a);
				m = m.set([k]);
				b.switchable && a.switchable && m.setAttr("cursor", "pointer");
				void 0 !== a.id && d.setCN(c, m, "legend-item-" + a.id);
				d.setCN(c, m, a.className, !0);
				g = b.switchType;
				var x;
				g && "none" != g && 0 < n && ("x" == g ? (x = b.createX(), x.translate(n / 2, n / 2)) : x = b.createV(), x.dItem = a, !0 !== a.hidden ? "x" == g ? x.hide() : x.show() : "x" != g && x.hide(), b.switchable || x.hide(), b.addListeners(x, a), a.legendSwitch = x, m.push(x), d.setCN(c, x, "legend-switch"));
				g = b.color;
				a.showBalloon && b.textClickEnabled && void 0 !== b.selectedColor && (g = b.selectedColor);
				b.useMarkerColorForLabels && (g = h);
				!0 === a.hidden && (g = b.markerDisabledColor);
				h = d.massReplace(b.labelText, {
					"[[title]]": a.title
				});
				void 0 !== b.tabIndex && (m.setAttr("tabindex", b.tabIndex), m.setAttr("role", "menuitem"), m.keyup(function(c) {
					13 == c.keyCode && b.clickMarker(a, c)
				}));
				c.accessible && b.accessibleLabel && (r = d.massReplace(b.accessibleLabel, {
					"[[title]]": a.title
				}), c.makeAccessible(m, r));
				r = b.fontSize;
				k && (n <= r && (n = n / 2 + b.ly - r / 2 + (r + 2 - n) / 2 - p, k.translate(t, n), x && x.translate(x.x, n)), a.legendEntryWidth = k.getBBox().width);
				var y;
				h && (h = d.fixBrakes(h), a.legendTextReal = h, y = b.labelWidth, y = isNaN(y) ? d.text(b.container, h, g, c.fontFamily, r, "start") : d.wrappedText(b.container, h, g, c.fontFamily, r, "start", !1, y, 0), d.setCN(c, y, "legend-label"),
					y.translate(b.lx + q, b.ly), m.push(y), b.labelDx = q, c = y.getBBox().width, b.maxLabelWidth < c && (b.maxLabelWidth = c));
				b.entries[b.index] = m;
			...
			if (a)
				for (c = 0; c < a.length; c++) {
					...
					if (e) f.value ? h.text(f.value) : h.text("");
						...
						f.legendColorFunction && (n = f.legendColorFunction(l, g, f.periodDataItem, f.periodPercentDataItem));
						h.text(g);
						this.useMarkerColorForValues && h.setAttr("fill", n);
						if (this.useMarkerColorForLabels) {
							if (h = f.legendMarker) h.setAttr("fill", n), h.setAttr("stroke", n);
