...
class PhpExcelComponent extends Component {
    ...
    public function setActiveSheet($sheet) {
        ...
        $this->_maxCol = PHPExcel_Cell::columnIndexFromString($this->_xls->setActiveSheetIndex($sheet)->getHighestColumn());
        $this->_row = 1;
        return $this;
    }
     ...
     * Get total row count
     *
     * @return int $maxRow number of row
     * @return $this for method chaining
     */
     ...
     */
    public function getTableRowData($max = 100) {
        ...
        for ($col = 0; $col < $max; $col++) {
			if (PHPExcel_Shared_Date::isDateTime($this->_xls->getActiveSheet()->getCellByColumnAndRow($col, $this->_row))) {
				$data[] = PHPExcel_Style_NumberFormat::toFormattedString($this->_xls->getActiveSheet()->getCellByColumnAndRow($col, $this->_row)->getValue(), 'YYYY-MM-DD');
			} else {
				$value = $this->_xls->getActiveSheet()->getCellByColumnAndRow($col, $this->_row)->getValue();
				if(isset($value[0]) == '=') {
					$value = $this->_xls->getActiveSheet()->getCellByColumnAndRow($col, $this->_row)->getCalculatedValue();
				}
            	$data[] = $value;
			}
		}
     ...
     * @return array row contents
     */
    public function getTableData($startRow = 1, $maxCol = null) {
		$this->_row = $startRow;
		if(!empty($maxCol)) {
			$this->_maxCol = $maxCol;
		}
		...
		$data = array();
		while($this->_row <= $this->_maxRow) {
			$data[] = $this->getTableRowData($this->_maxCol);
		}
        return $data;
    }
