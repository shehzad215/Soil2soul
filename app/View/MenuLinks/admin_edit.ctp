...
<div class="portlet box yellow">
	<div class="portlet-title">
		...
		<?php
			...
			$dropdownMenu = [];
			$dropdownMenuLink[] = $this->Bs->link(__('View'), array('action'=>'view', $this->Form->value('MenuLink.id')), array('icon'=>'search', 'class'=>'btn white btn-sm btn-outline'));
			if($isLoggedIn && $userRole['delete']) : 
				$dropdownMenuLink[] = $this->Bs->link(__('Delete'), array('action'=>'delete', $this->Form->value('MenuLink.id')), array('icon'=>'trash-o', 'class'=>'btn white btn-sm btn-outline', 'data-toggle'=>'modal-manager'));
			endif;			
		...
		<?php
			...
			echo $this->Html->div('tab-box',
				$this->Html->div('row',
					...
					$this->Bs->input('parent_id', array('col'=>'6', 'empty'=>'Root', 'options'=>$parentMenuLinks,'class'=>'addNewParentId', 'append'=>array('button'=>'Add New')))
				)	
			);
			echo $this->Html->div('tab-box', 
			$this->Html->div('row',
...
<?php $this->start('appScript'); ?>
	...
	var menuId = $('#MenuLinkMenuId').val();
   	frontEndSetup(menuId);
	...
	var IsFileVal = "<?php echo $this->request->data['MenuLink']['is_file'];?>";
	setupFile(IsFileVal);
	...
	var IsOLinkVal = "<?php echo $this->request->data['MenuLink']['is_outer_link'];?>";
	setupLink(IsOLinkVal);
	$('.outUrlLink').change(function(){
		var IsOLinkVal = $(this).val();
		setupLink(IsOLinkVal);
	});
	function setupLink(IsOLinkVal){
		if(IsOLinkVal == '1'){
			$('.FrontendField').show();
			$('.OuterURL').show();
		}else{
