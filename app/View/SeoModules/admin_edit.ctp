<?php 
	...
	echo $this->Html->tag('li', $this->Html->link(__('Admin Edit Seo Module'), array('controller'=>'seo_modules', 'action'=>'admin_edit', $this->Form->value('SeoModule.id')), array()), array('class'=>'active', 'typeof'=>'v:Breadcrumb') );
	echo $this->end();
?>
...
<div class="portlet box yellow">
	<div class="portlet-title">
		<div class="caption">
            <?php echo __('Admin Edit Seo Module'); ?> <?php echo $this->Bs->loader(); ?>        </div>
		...
		<?php
			...
			$dropdownMenuLink[] = $this->Bs->link(__('View'), array('action'=>'view', $this->Form->value('SeoModule.id')), array('icon'=>'search', 'class'=>'btn default btn-sm', 'data-toggle'=>'modal-manager'));
			if($isLoggedIn && $userRole['delete']) : 
				$dropdownMenuLink[] = $this->Bs->link(__('Delete'), array('action'=>'delete', $this->Form->value('SeoModule.id')), array('icon'=>'trash-o', 'class'=>'btn default btn-sm', 'data-toggle'=>'modal-manager'));
			endif;
	...
	<div class="portlet-body form">
        ...
        <div class="form-body">
		<?php
			$mainImgPath =  $this->webroot.'files/seo_module/meta_image/'.$this->request->data['SeoModule']['id'].'/'.$this->request->data['SeoModule']['meta_image'];
				...
				$this->html->div('row',
					$this->html->div('col-sm-3',
						$this->html->div('passportphoto',
							(empty($this->request->data['SeoModule']['meta_image'])) ? $this->html->image('no-image.png',['id'=>'blah','class'=>'imageBanner']).
								$this->Bs->input('SeoModule.meta_image',['type'=>'file','icon'=>'file','id'=>'imgupload','label'=>false]).
							...
							$this->Bs->input('SeoModule.meta_image',['type'=>'file','id'=>'imgupload','label'=>false]).
							$this->html->tag('button','Upload Image',['type'=>'button','class'=>'btn btn-default btn-file uploadImageBtn','id'=>'OpenImgUpload']).
							'</div></div>'
						)
					).	
