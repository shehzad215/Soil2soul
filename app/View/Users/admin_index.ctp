...
<div class="col-md-12">
    <div class='portlet box yellow'>
        <div class="portlet-title">
            <div class="caption"><?php echo __('Users'); ?> <?php echo $this->Bs->loader(); ?></div>
            <div class="actions">
						    		<?php
														...
														if($isLoggedIn && $userRole['add']) :
															$dropdownMenuLink[] = $this->Bs->link(__('New User'), array('action' => 'add'), array('icon'=>'plus', 'class'=>'btn btn-sm white btn-outline'));
														else : 
                 	...
                 	echo $this->Bs->create('User', array('class'=>'form-vertical','id'=>'preEnquirySearchForm'));
                 		echo $this->Html->div('tab-box',
                    		  $this->Html->div('row',
                    		  		...
                    		  		$this->Bs->input('username', array('options' =>$usernameArr,'empty'=>'All','required'=>false,'col'=>'2','value'=>$usernameValue)).

                    		  		$this->Bs->input('id', array('options' =>$nameArr,'empty'=>'All','label'=>'Name','required'=>false,'col'=>'2','value'=>$nameValue)).
            ...
            <div class="table-responsive">
                ...
                <?php foreach ($users as $user): ?>
																		<tr>
																			...
																			<td class='text-center'><input data-set-row="countries-checkbox"  type="checkbox" name="check[]" value="<?php echo h($user['User']['active']); ?>" class ='activeClass'  data-value="<?php echo $user['User']['id'];?>" <?php  if($user['User']['active'] == 1){ ?> checked <?php }?>>&nbsp;</td>
																			...
																			<?php
																				...
																				$dropdownMenuLink[] = $this->Bs->link(__('View'), array('controller'=>'users', 'action'=>'view', $user['User']['id']), array('icon'=>'search', 'class'=>'btn btn-warning linkrow', 'data-toggle'=>'modal-manager') );
...
$(function() {
  <?php $this->start('appScript'); ?> 
    ...
    $('[id*="preEnquirySearchForm"] :input').change(function(e) {
      e.preventDefault();
      var baseUrl = '<?php echo $this->Html->url('/admin/users/index');?>';
 ...
 $('.activeClass').change(function(){
  ...
  var usersid = $(this).attr('data-value');
   $.ajax({
          type:"POST",
          url: "<?php echo $this->Html->url(['controller'=>'users','action'=>'is_active']); ?>",
          data: {"usersid":usersid,"checkedid":checkedid} 
        });
 });
