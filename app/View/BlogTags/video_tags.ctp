...
<?php foreach ($videos as $key => $video) { //debug($video);die;
	if(!empty($video['Video'])) { 
	foreach ($video['Video'] as $key => $video) { ?>
<div class="videoimgbox">
   <div class="row">
        <div class="col-md-5 col-sm-4">
          <a href="javascript:void(0);" onclick="openModal('<?php echo $video['video_link'].'?&autoplay=1&mute=1';?>');">
            <span class="videoimgboxplay"></span>
            <?php echo $this->Bs->image('Video.image_file',$video,['alt'=>$video['title'],'class'=>'img-responsive']); ?>
            <!-- <img src="<?php //echo $this->webroot;?>img/a_journey_beyond.jpg" alt="" class="img-responsive" /> -->
            </a>    
            </div>
        <div class="col-md-7 col-sm-8">
    <h4><?php echo $video['title'] ?></h4>
<?php echo strlen($video['description']) > 450 ? substr($video['description'], 0, 450) . '... <a href="#" class="prevent OurVideoModel" data_id="' . $video['id'] . '">Read More</a>' : $video['description']; ?>
        </div>    
        </div>
...
<div class="row">
<?php //debug($randomVideos); ?>
<?php foreach ($randomVideos as $key => $randomVideo) { //debug($randomVideo['Video']['video_link']);die; ?>
