...
class GoogleMapV3Helper extends AppHelper {
	 ...
	 */
	public function apiUrl($sensor = true, $api = null, $language = null, $append = null) {
	...
	public function map($options = []) {
		...
		$this->settings['map'] = array_merge($this->settings['map'], ['zoom' => $this->settings['zoom'], 'lat' => $this->settings['lat'], 'lng' => $this->settings['lng'], 'type' => $this->settings['type']], $options);
		...
		if (!empty($this->settings['autoScript']) && !$this->_apiIncluded) {
			$res = $this->Html->script($this->apiUrl(), ['inline' => $this->settings['inline']]);
		...
		if (false && !empty($this->settings['autoScript']) && !$this->_gearsIncluded) {
			$res = $this->Html->script($this->gearsUrl(), ['inline' => $this->settings['inline']]);
			if ($this->settings['inline']) {
				$result .= $res . PHP_EOL;
			}
		}
	...
	public function addMarker($options) {
		...
		if (isset($options['propertyId'])) {
			$params['propertyId'] = json_encode($options['propertyId']);
		}
...
*/
	 ...
	 * @return void
	 */
	public function addCircle($options = []) {
		$defaults = $this->settings['circle'];
...
function geocodeAddress(address) {
	geocoder.geocode({'address': address}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			...
			x" . static::$markerCount . " = new google.maps.Marker({
				position: results[0].geometry.location,
				" . $this->_toObjectParams($options, false, false) . "
			});
			gMarkers" . static::$mapCount . " .push(
				x" . static::$markerCount . "
			);
			...
			if (!isset($options['address'])) {
				throw new CakeException('Either use lat/lng or address to add a circle');
			}
			$position = 'geocodeAddress(\'' . h($options['address']) . '\')';
		} else {
			$position = "new google.maps.LatLng(" . $options['lat'] . "," . $options['lng'] . ")";
		...
		$circle = "
			var c" . static::$circleCount . " = new google.maps.Circle({
                map: ".$this->name().",
				center: " . $position . ",
				editable: true,
                geodesic: true, 
	...
	/**
	 * Add a circle listen event for a marker on click.
	 *
	 * @return void
	 */
	public function addCircleEvent() {
		$this->map .= "            
			google.maps.event.addListener(c0, 'radius_changed', function() {
                inCircle();
            })
            
            google.maps.event.addListener(c0, 'drag', function() {
                inCircle();
            })
            
            var inCircle = function() {
                var activeIcon = '".Router::url('/img/circle-red.png')."';
                var inActiveIcon = '".Router::url('/img/circle-grey.png')."';
                
                var cPos = c0.getCenter();
                var cRadius = c0.getRadius() / 1000;
                
                $.each(gMarkers0, function(index, marker) {
                    var propId = marker.propertyId;
                    var mPos = marker.getPosition();
                    
                    var property = $('#property-list '+propId)
                    var distance = getDistance(mPos, cPos);
//                    console.log(cRadius);
//                    console.log(distance);
//                    console.log((distance > cRadius));
                    if (distance > cRadius) {
                ...
                var totalCount = $('#property-list .property-item').length;
                var showingCount = $('#property-list .property-item:not(.hide)').length;
...
\'';
    ...
    public function getNearByPlaces($location, $type, $limit = 5) {
        $apiKey = "AIzaSyDY3qC7U5n0PaD277BlQ4lGthMPwmgDwPk";
        $url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='.
            $location['lat'].','.$location['lng'].'&types='.$type
            .'&sensor=false&rankby=distance&key='.$apiKey;
        ...
        return array_slice($responseArr['results'], 0, $limit);
    }
    
    public function getNearByPlacesWithDistance($location, $type, $limit = 5) {
        $arr = [];
        $results = $this->getNearByPlaces($location, $type, $limit);
        foreach($results as $result) {
           $arr[] = array($result['name'], $this->distance($location, Hash::get($result, 'geometry.location')).' KM');
        }
