 ...
 */
    public function getDatesFromRange($start, $end) {
        ...
        foreach($period as $date) {
            $array[] = $date->format(Configure::read('dateFormatApp'));
        }
 ...
 */
    ...
    public function price_block($attraction, $userWishlistAttractionsList ,$wishlists = false, $slug = '', $flagElement ='') {

        $sellingPrice = $productPriceCount =  $discount_amount = 0;
        ...
        if(!empty($discount_amount)) {
            ...
            $html .= $this->Html->div('textorange22boldprice currencydiv',
                $this->Html->tag('span', $discount_amount.'</br>', ['class'=>'pricetextdicount', 'style'=>'text-align:center !important;']).
                //'&nbsp; '.$sellingPrice
                ''.$sellingPrice
            );
        } else { 
...
/*
        ...
        'attractionslug' => $slug,'user' =>false], 
                ['class'=>'learnmoretbtn readMoreHideClass', 'target' => '_blank']
            ); */
            ...
            if(Hash::get($attractionfinal, '0.AttractionProductPriceGroup.0.AttractionProductPrice.0')) {
                foreach ($attractionfinal as $key => &$attractionProduct) {
                    ...
                    foreach ($attractionProduct['AttractionProductPriceGroup'] as $attractionProductPriceGroupkey => $attractionProductPriceGroup){
                        ...
                        if(!empty($attractionProductPriceGroup['AttractionProductPrice'])){
                ...
                if(!empty($sortProductDisplayPrice)) {
                    ...
                    //for Agent is from API
                    if(CakeSession::check('Agent.sessions')) {
                        $selling_priceTitle = $selling_price_view = $selling_price;
                    }else{
                    ...
                    return $response = [
                        ...
                        'selling_price_view' => $selling_price_view,
    ...
    public function getPriceTitle($value = '', $signRequired = '') {
       ...
       if($signRequired){
            ...
            return ($sessionCurrency['Currency']['blank'] == true) ? 
                $isoCode.' <strong class="priceOnChangePrivate">'.$value.'</strong>': 
                $isoCode.'<strong class="priceOnChangePrivate">'.$value.'</strong>';
       }
