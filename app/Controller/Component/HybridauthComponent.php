...
class HybridauthComponent extends Component {
    ...
    protected function normalizeSocialProfile($provider){
        ...
        $socialProfile['UserSocialProfile']['social_network_name'] = $provider;
        $socialProfile['UserSocialProfile']['social_network_id'] = $incomingProfile['identifier'];
        $socialProfile['UserSocialProfile']['email'] = $incomingProfile['email'];
        $socialProfile['UserSocialProfile']['display_name'] = $incomingProfile['displayName'];
        $socialProfile['UserSocialProfile']['first_name'] = $incomingProfile['firstName'];
        $socialProfile['UserSocialProfile']['last_name'] = $incomingProfile['lastName'];
        $socialProfile['UserSocialProfile']['link'] = $incomingProfile['profileURL'];
        $socialProfile['UserSocialProfile']['picture'] = $incomingProfile['photoURL'];
        $socialProfile['UserSocialProfile']['created'] = date('Y-m-d h:i:s');
        $socialProfile['UserSocialProfile']['modified'] = date('Y-m-d h:i:s');
            
        // twitter does not provide email so we need to build someting
        if($provider == 'Twitter'){
            $names = explode(' ', $socialProfile['UserSocialProfile']['first_name']);
            $socialProfile['UserSocialProfile']['first_name'] = $names[0];
            $socialProfile['UserSocialProfile']['last_name'] = (count($names)>1 ? end($names) : '');
            $socialProfile['UserSocialProfile']['display_name'] = $socialProfile['UserSocialProfile']['first_name'] .'_'. $socialProfile['UserSocialProfile']['last_name'];
            $socialProfile['UserSocialProfile']['email'] = $socialProfile['UserSocialProfile']['display_name'] .'@Twitter.com';
        }
