...
public function scholar_details(){
	...
	$scholarId = $this->request->data['scholarId'];
	$journeyId = $this->request->data['journeyId'];

	// debug($scholarId);

	$schoLarDetails = $this->OurScholarDetail->OurTeam->find('first',['conditions'=>['OurTeam.id'=>$scholarId],'contain'=>['OurScholarDetail'=>['conditions'=>['OurScholarDetail.our_team_id'=>$scholarId,'OurScholarDetail.our_journy_id'=>$journeyId]]]]);
	...
	// debug($schoLarDetails);exit;
	/*Set Compact*/
	$this->set(compact('schoLarDetails'));
