<?php
App::uses('TelecallingLog', 'Model');

/**
 * TelecallingLog Test Case
 *
 */
class TelecallingLogTest extends CakeTestCase {

/**
 * Fixtures
 *
 * @var array
 */
	public $fixtures = array(
		'app.telecalling_log',
		'app.market',
		'app.country',
		'app.zone',
		'app.country_detail',
		'app.timezone',
		'app.user',
		'app.role',
		'app.menu_link',
		'app.menu',
		'app.menu_links_role',
		'app.menu_links_user',
		'app.salutation',
		'app.competitor_contact',
		'app.competitor',
		'app.city',
		'app.bank_account',
		'app.company',
		'app.event_contact',
		'app.event',
		'app.event_type',
		'app.event_domain',
		'app.event_organizer',
		'app.contact',
		'app.contact_type',
		'app.visitor_type',
		'app.delegate_visitor_enquiry',
		'app.user_activation_detail',
		'app.delegate_role',
		'app.event_meeting',
		'app.event_meeting_status',
		'app.booth_type',
		'app.event_meeting_status_history',
		'app.stay_detail',
		'app.meeting_type',
		'app.crm_meeting_status',
		'app.source_type',
		'app.enquiry_source_type',
		'app.delegate_visitor_enquiry_detail',
		'app.partner',
		'app.partner_contact',
		'app.designation',
		'app.supplier_contact',
		'app.supplier',
		'app.supplier_type',
		'app.market_news',
		'app.supplier_attachment',
		'app.supplier_image',
		'app.event_supplier',
		'app.email_campaign',
		'app.lost_reason',
		'app.crm_quote',
		'app.currency',
		'app.crm_quote_attachment',
		'app.crm_assigned_lead',
		'app.crm_stage',
		'app.crm_followup',
		'app.crm_sub_stage',
		'app.crm_followup_attachment',
		'app.crm_assigned_leads_user',
		'app.delegate_swap_user_history',
		'app.finance',
		'app.finance_api_detail',
		'app.api',
		'app.api_type',
		'app.product',
		'app.product_domain',
		'app.product_attachment',
		'app.event_product',
		'app.product_presentation',
		'app.finances_product',
		'app.visitor_detail',
		'app.delegate_enquiry_schedule',
		'app.delegate_enquiry_reschedule',
		'app.crm_schedule_demo',
		'app.demo_status',
		'app.crm_schedule_demos_user',
		'app.delegate_visitor_enquiries_event_product',
		'app.stall_designer',
		'app.stall_designers_country',
		'app.stall_designer_detail',
		'app.stall_designer_image',
		'app.contact_detail',
		'app.event_organizer_detail',
		'app.department',
		'app.organizer_event_list',
		'app.events_event_organizer',
		'app.event_attachment',
		'app.event_content',
		'app.event_content_category',
		'app.event_competitor',
		'app.event_guideline',
		'app.stall_detail',
		'app.stall_type',
		'app.website_stall_detail',
		'app.actual_stall_detail',
		'app.budget_detail',
		'app.budget_type',
		'app.event_speed_net',
		'app.network_type',
		'app.event_speed_nets_user_activation_detail',
		'app.event_timetable_setup',
		'app.competitor_attachment',
		'app.competitor_product',
		'app.user_category',
		'app.user_detail',
		'app.markets_country'
	);

/**
 * setUp method
 *
 * @return void
 */
	public function setUp() {
		parent::setUp();
		$this->TelecallingLog = ClassRegistry::init('TelecallingLog');
	}

/**
 * tearDown method
 *
 * @return void
 */
	public function tearDown() {
		unset($this->TelecallingLog);

		parent::tearDown();
	}

}
